...
class MyModel:
    """
    Faster multilingual next-character model.

    Design:
    - dense self-supervised character n-gram training from unlabeled contexts
    - interpolated/backoff character LM
    - exact-context memorization from labeled examples
    - word-fragment completion model
    - script-aware prior for multilingual text
    - lightweight count-based ensemble only (slow reranker removed)

    This is intentionally much cheaper than the previous validation-reranker version.
    """
    ...
    def _train_on_sequence(self, text, weight=1.0):
        ...
        for i, nxt in enumerate(truncated):
            ...
            self._update_script_models(context.replace(self.BOS, ''), nxt, weight=weight)
    ...
    def _candidate_pool(self, context, ngram, exact, word, boundary, script, limit=48):
        merged = Counter()
        for source, wt in ((ngram, 1.0), (exact, 1.2), (word, 0.8), (boundary, 0.15), (script, 0.35)):
        ...
        if len(ranked) < limit:
            for ch, _ in self.global_counts.most_common(limit * 2):
                if ch not in self.DISALLOWED_OUTPUT_CHARS and ch not in ranked:
                    ranked.append(ch)
                if len(ranked) >= limit:
                    break
        return ranked

    def _score_candidate(self, context, ch, ngram, exact, word, boundary, script):
        score = 0.0
        score += self.weights['ngram'] * ngram.get(ch, 0.0)
        score += self.weights['exact'] * exact.get(ch, 0.0)
        score += self.weights['word'] * word.get(ch, 0.0)
        score += self.weights['boundary'] * boundary.get(ch, 0.0)
        score += self.weights['script'] * script.get(ch, 0.0)
        score += self.weights['global'] * self.global_probs.get(ch, 0.0)

        if context:
            last = context[-1]
            if ch == last:
                score *= self.weights['same_last_penalty']
            if len(context) >= 2 and context[-1] == context[-2] == ch:
                score *= self.weights['double_penalty']
    ...
    def _top_guesses(self, context, n=3):
        ...
        candidates = self._candidate_pool(context, ngram, exact, word, boundary, script, limit=48)
        scored = []
        for ch in candidates:
            scored.append((self._score_candidate(context, ch, ngram, exact, word, boundary, script), ch))
